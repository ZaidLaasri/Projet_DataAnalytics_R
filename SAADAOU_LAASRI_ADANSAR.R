  ...
  # Vérifiez si log_debcred existe dans test_data, sinon créez-la
  if (!"log_debcred" %in% colnames(test_data)) {
    test_data$log_debcred <- log(test_data$debcred + 1)
    cat("Colonne 'log_debcred' créée dans test_data.\n")
  }
  
  # Imputation des valeurs manquantes dans test_data
  test_data$log_debcred[is.na(test_data$log_debcred)] <- mean(test_data$log_debcred, na.rm = TRUE)
  
  # Vérification finale
  cat("Colonnes manquantes dans test_data par rapport à train_data :\n")
